...
app.post("/", function (req, res) {
    if (req.body.buttonData == "hostel") res.redirect("/hostelauth");

    if (req.body.buttonData == "guard") res.redirect("/guardauth");
});
...
app.post("/hostelauth", function (req, res) {
    hostelAuthenticationData.findOne(
                ...
                bcrypt.compare(req.body.password, result.hashPassword, function (err, check) {
                        if (err) {
                            ...
                            res.render("interface", {
                                interfaceText: "Outpass Generation",
                                interfaceSubText: "Enter UID",
                                messageText: "",
                            });
                        } else {
                            res.render("authpage", {
...
app.post("/hostel/features", (req, res) => {
    const buttonData = req.body.button;
    console.log(buttonData);
    if (buttonData == "genpass") {
        hostelData.find({}, (err, details) => {
            if (err) {
                console.log(err);
        ...
        guardData.find({}, (err, details) => {
            if (err) {
                console.log(err);
            } else res.redirect("/hostel/features/verrifiedpass");
        });
...
app.get("/hostel/features/verrifiedpass", (req, res) => {
    guardData.find({}, (err, details) => {
        if (err) {
            console.log(err);
        } else res.render("outpassList", { list: details });
...
app.post("//hostel/features/verrifiedpass", (req, res) => {
...
app.post("/hostel", function (req, res) {
    ...
    hostelData.findOne(
        { uidData: dataCameFromPost.toUpperCase() },
        function (err, result) {
            if (err) console.log(err);
                ...
                res.render("hostelResult", {
                    studentName: foundData.name,
                    uidData: foundData.uidData,
                    mobileNo: foundData.mobileNo,
                    gurdianNo: foundData.gurdianNo,
                    hostelName: foundData.hostelDetails.hostelName,
                    roomNo: foundData.hostelDetails.roomNo,
                });
            }
        }
    );
        ...
        function (err, result) {
            if (err) {
                ...
                res.render("interface", {
                    interfaceText: "Outpass Generation",
                    interfaceSubText: "Enter UID",
                    messageText: "Outpass already created",
                });
            }
        }
    );
...
app.post("/guardauth", function (req, res) {
    guardAuthenticationData.findOne(
        { username: req.body.username },
        function (err, result) {
            if (err) {
                console.log(err);
                    ...
                    function (err, check) {
                        if (err) {
                            ...
                            res.render("interface2", {
                                interfaceText: "Outpass Verrification",
                                interfaceSubText: "Enter UID",
                                messageText: "",
                            });
                        } else {
...
app.post("/guardResult", function (req, res) {
    acceptedPass.findOne(
        { uidData: req.body.uidData.toUpperCase() },
        function (err, result) {
            if (err) {
                console.log(err);
                res.render("interface2", {
